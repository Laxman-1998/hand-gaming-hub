...
// FORCING THE VISION ENGINE TO USE GLOBAL ABSOLUTE CDN PERMISSIONS
let hands = new Hands({ 
    locateFile: (file) => {
    ...
    const interval = setInterval(() => {
        if (count > 0) {
            ...
            if (count === 1 && !isMultiplayerActive) {
                ...
                if (userScore >= 3) {
                    const r = playerMoveHistory.filter(m => m === 'ROCK').length, p = playerMoveHistory.filter(m => m === 'PAPER').length, s = playerMoveHistory.filter(m === 'SCISSORS').length;
            ...
            if (isMultiplayerActive) {
                ...
                statusMain.innerText = "SUBMITTED!"; statusSub.innerText = "WAITING FOR RIVAL REVEAL...";
                evaluateMultiplayerNetworkMatch();
            } else {
...
window.addEventListener('beforeunload', () => { if (wsChannel) { wsChannel.close(); } });
