...
function triggerLocalAiArena() {
    ...
    document.getElementById(
        'opponent-deck-label'
    ).innerText = "AI OPPONENT";

    document.getElementById(
        'opponent-score-label'
    ).innerText = "AI CORE";

    aiIntelHTML.innerText =
        "LOCAL TRAINING ARENA";
...
function onResults(results) {
    ...
    if (
        ...
        detectedGestureHTML.innerText =
            currentDetectedGesture;

        if (
            isArenaActive &&
            !isCountingDown &&
            currentDetectedGesture !== "Analyzing..."
        ) {
...
function startBattleCountdown() {
    ...
    countdownNumberHTML.style.display =
        "block";
        ...
        setInterval(() => {
            ...
            if (count > 0) {
                ...
                if (isMultiplayerActive) {
                    ...
                    if (
                        !currentDetectedGesture.includes(" ")
                    ) {
                    ...
                    myLastSubmittedMove =
                        currentDetectedGesture.split(" ")[1];
...
function evaluateMultiplayerNetworkMatch() {
    ...
    aiEmojiHTML.innerText =
        opponentLastSubmittedMove === "ROCK"
            ? "✊"
            : opponentLastSubmittedMove === "PAPER"
            ? "🖐️"
            : "✌️";
...
async function startCamera() {
    ...
    try {
        ...
        if (urlParams.get('room')) {
            ...
            lobbyLoaderText.innerText =
                "Joining Multiplayer Arena...";
